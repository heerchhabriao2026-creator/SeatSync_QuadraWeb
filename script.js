...
function renderAnalytics() {
    ...
    if (occupancyCanvas) {
        const ctx1 = occupancyCanvas.getContext("2d");
        ...
        occupancyChartInstance = new Chart(ctx1, {
            ...
            data: {
                ...
                datasets: [{
                    label: "Occupancy %",
                    data: [15, 45, 78, 92, 85, 60, 30],
                    borderColor: "#6a4fd6",
                    backgroundColor: "rgba(106, 79, 214, 0.1)",
                    fill: true,
                    tension: 0.4
                }]
            },
    ...
    // Convert database values safely to lowercase
    const quietCount = allSeats.filter(
        seat => String(seat.zone || "").toLowerCase() === "quiet"
    ).length;

    const groupCount = allSeats.filter(
        seat => String(seat.zone || "").toLowerCase() === "group"
    ).length;

    const pcCount = allSeats.filter(
        seat => String(seat.zone || "").toLowerCase() === "pc"
    ).length;

    console.log("Zone counts:");
    console.log("Quiet:", quietCount);
...
async function checkDepartures() {
    ...
    for (const seat of allSeats) {
        ...
        // 5 minutes or less → show countdown reminder
        if (
            diff > 0 &&
            minutesLeft <= 5 &&
            seat.departure_stage !== "reminder"
        ) {
        ...
        // Time finished → overdue
        if (
            diff <= 0 &&
            seat.departure_stage !== "overdue"
        ) {
            await supabaseClient
                .from("seats")
...
// Update the displayed countdown regularly
    setInterval(() => {
